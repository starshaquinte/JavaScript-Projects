...
const Calculator = {
    ...
    First_Operand: null,
    //This checks wether or not the second has been inputted by the user.
    Wait_Second_Operand: false,
    //This will hold the operator, we set to null for now.
    operator: null,
};

//This modifies values eaach time a button is clicked on.
...
function Input_Decimal(dot) {
    //This ensures that accident clicking of the decimal point doesn't
    //cause bugs in the opeartion.
...
//This section habdles operators 
function Handle_Operator(Next_Operator) {
    const {First_Operand, Display_Value, operator} = Calculator;
    //When an opeartor key is pressed we convert the current number
    ...
    }  else if (operator)  {//Check if an opeartor already exist}
...
keys.addEventListener('click', (event) => {
    ...
    //that was clicked.
    const { target } = event;
    //If the element that was clicked on is not  button, exit the function.
